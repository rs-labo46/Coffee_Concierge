...
func TestSessionUsecase_GetSession_UserOwned(t *testing.T) {
	...
	uc := NewSessionUsecase(sessionRepoMock{
		getSessionByIDFn: func(id uint) (*entity.Session, error) { return &entity.Session{ID: id, UserID: &uid, Status: entity.SessionActive}, nil },
		...
		listSuggestionsFn: func(sessionID uint) ([]entity.Suggestion, error) { return []entity.Suggestion{{SessionID: sessionID}}, nil },
	...
	if out.Session.ID != 9 || len(out.Turns) != 1 || out.Pref.SessionID != 9 { t.Fatalf("unexpected output:%+v", out) }
}
...
func TestSessionUsecase_CloseSession_GuestConflict(t *testing.T) {
	...
	if err != ErrConflict { t.Fatalf("expected ErrConflict, got:%v", err) }
}

func TestSessionUsecase_ListHistory_User(t *testing.T) {
	uc := NewSessionUsecase(sessionRepoMock{listHistoryFn: func(q port.HistoryQ) ([]entity.Session, error) {
	...
	if err != nil || len(out) != 1 || out[0].ID != 1 { t.Fatalf("unexpected output:%+v err=%v", out, err) }
}
