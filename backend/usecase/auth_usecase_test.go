...
func TestAuthUsecase_Signup(t *testing.T) {
	...
	if out.User.ID != 1 || out.User.PassHash != "hashed" {
		t.Fatalf("unexpected user:%+v", out.User)
	}
	if mailer.verifyTo != "u@example.com" || mailer.verifyToken != "plain-token" {
		t.Fatalf("mailer not called:%+v", mailer)
	}
}

func TestAuthUsecase_Login_PasswordMismatch(t *testing.T) {
	uc := NewAuthUsecase(
		userRepoMock{getByEmailFn: func(email string) (*entity.User, error) {
			return &entity.User{ID: 1, Email: email, PassHash: "hashed", Role: entity.RoleUser, TokenVer: 1}, nil
		}},
		emailVerifyRepoMock{},
		pwResetRepoMock{},
		rtRepoMock{},
