...
func TestSavedUsecase_ListAndDelete(t *testing.T) {
	uc := NewSavedUsecase(
		savedRepoMock{
			listFn: func(q repository.SavedListQ) ([]entity.SavedSuggestion, error) {
				return []entity.SavedSuggestion{{UserID: q.UserID}}, nil
			},
			getFn: func(userID uint, suggestionID uint) (*entity.SavedSuggestion, error) {
				return &entity.SavedSuggestion{UserID: userID, SuggestionID: suggestionID}, nil
			},
		},
		sessionRepoMock{},
		&auditRepoMock{},
	...
	if err != nil || len(list) != 1 || list[0].UserID != 7 {
		t.Fatalf("unexpected list:%+v err=%v", list, err)
	}
	if err := uc.Delete(DeleteSavedIn{Actor: entity.Actor{UserID: 7}, SuggestionID: 3}); err != nil {
