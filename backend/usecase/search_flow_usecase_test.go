...
func TestSearchFlowUsecase_StartSession_Guest(t *testing.T) {
	...
	if out.Session.ID != 1 || out.SessionKey != "guest-key" { t.Fatalf("unexpected output:%+v", out) }
	if created == nil || created.UserID != nil || created.GuestExpiresAt == nil { t.Fatalf("guest session was not created correctly:%+v", created) }
}
...
func TestSearchFlowUsecase_StartSession_User(t *testing.T) {
	...
	if err != nil { t.Fatalf("unexpected error:%v", err) }
	if out.Session.ID != 2 || out.SessionKey != "" { t.Fatalf("unexpected output:%+v", out) }
	if created == nil || created.UserID == nil || *created.UserID != 9 { t.Fatalf("user session was not created correctly:%+v", created) }
}
